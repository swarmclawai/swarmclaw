...
description: Use on Zmey's local SwarmClaw instance when decomposing a large task into direct assigned worker tasks, especially app builds, multi-file plans, QA sweeps, or independent failure investigations that can run without shared write conflicts.
tags: [swarmclaw, tasks, parallel-agenting, coordination, qa]
---

# SwarmClaw Parallel Agenting

Turn one large target into observable SwarmClaw tasks with clear owners, bounded scopes, and review checkpoints.

This is SwarmClaw-local guidance. It adapts the public `subagent-driven-development` pattern to Zmey's local instance. The local operator manual, current handoff, `AGENTS.md`, and live runtime behavior win over this skill when they conflict.
...
- Read-only QA can run in parallel with implementation.
...
- Refresh Knowledge, install external skills, or pin/attach skills only after Zmey checkpoints that exact action.

## Default Workers

Confirm these still match the current instance before assignment:
...
If the target is vague, create a planning task first instead of starting implementation.
...
Prefer fewer high-quality tasks over many tiny tasks. Each task needs a concrete definition of done.
...
- early risk detection is useful
...
Do not retry, cancel, archive, or edit task state unless Zmey approved that action.
...
Worker ID: [exact stored worker id]
...
- installing external skills or pinning skills to agents
