...
function initDb() {
  ...
  return {
    ...
    search(query: string, agentId?: string, options: MemorySearchOptions = {}): MemoryEntry[] {
      ...
      // Apply MMR for diversity — uses embedding similarity when available, Jaccard text fallback otherwise.
      let out: MemoryEntry[] = []
      if (rerankMode !== 'lexical') {
        out = applyMMR(queryEmbeddingResult || null, salienceScored, MAX_MERGED_RESULTS, 0.7)
      } else {
