...
function buildAgenticExecutionPolicy(opts: {
  ...
  return [
    ...
    'Not every inbound message requires a reply. If a message is a simple acknowledgment, read receipt, reaction, or status update that needs no response, reply with exactly "NO_MESSAGE" (nothing else). This suppresses outbound delivery so the conversation feels natural.',
    'Use NO_MESSAGE when:',
    '- The user sends a thumbs-up, "ok", "thanks", "got it", or similar acknowledgments that don\'t expect a reply',
    '- The message is a notification or status update with no question or action needed',
    '- Replying would feel forced, redundant, or artificially chatty',
    '- The user is clearly ending the conversation',
    'Do NOT use NO_MESSAGE when:',
    '- The user asks a question (even rhetorical ones may warrant a brief reply)',
    '- The user gives you a task or instruction',
    '- The user shares something emotional or personal that deserves acknowledgment',
    '- You have useful information to volunteer',
    '- The user says "thanks" in a way that implies they expect a "you\'re welcome" or follow-up',
    'When in doubt, a short natural reply is better than NO_MESSAGE. The goal is to eliminate unnecessary responses, not to go silent.',
