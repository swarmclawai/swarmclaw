...
import { formatTokens, formatCost, formatDurationMs } from '@/lib/format-display'
...
export default function UsagePage() {
  ...
  const blockedTasks = useMemo(
    () => Object.values(tasks).filter((task) => (task.blockedBy?.length || 0) > 0).length,
    [tasks],
  )
  ...
  const insightCards = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        value: `${blockedTasks} blocked · ${failedTasks} failed`,
        hint: blockedTasks + failedTasks > 0 ? 'Operational overhead from live work' : 'No obvious workflow friction',
        tone: blockedTasks + failedTasks > 0 ? 'text-amber-400' : 'text-emerald-400',
      },
    ]
  }, [blockedTasks, data, failedTasks])
                    ...
                    .map(([name, h], idx) => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-[11px] text-text-3">{timeAgo(h.lastUsed, Date.now())}</span>
                        </div>
