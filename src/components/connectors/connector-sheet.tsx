  ...
  configFields: { key: string; label: string; placeholder: string; help?: string }[]
    ...
    },
    {
      ...
      configFields: [
        ...
        { key: 'nodeId', label: 'Client Label', placeholder: 'agent-ember', help: 'Optional display label shown in OpenClaw presence metadata.' },
...
export function ConnectorSheet() {
  ...
  const inputClass = "w-full px-4 py-3 rounded-[12px] border border-white/[0.08] bg-card text-text text-[14px] outline-none transition-all placeholder:text-text-3/50 focus:border-white/[0.15]"
    ...
    <BottomSheet open={open} onClose={() => { setOpen(false); setEditingId(null) }} wide>
        ...
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <span className={`w-2 h-2 rounded-full ${editing.status === 'running' ? 'bg-green-400 shadow-[0_0_6px_rgba(74,222,128,0.5)]' :
      ...
      <div className="mb-6">
        <button
          ...
          className="flex items-center gap-2 text-[13px] font-600 bg-transparent border-none cursor-pointer p-0"
          style={{ fontFamily: 'inherit' }}
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"
            className={`transition-transform text-primary ${showSetup ? 'rotate-90' : ''}`}>
            <polyline points="9 18 15 12 9 6" />
          </svg>
          <span className="text-primary hover:opacity-80 transition-opacity">{platformConfig.label} Setup Guide</span>
        </button>
        {showSetup && (
      ...
      {/* Bot token credential */}
      {
        platform !== 'whatsapp' && (
          <div className="mb-6">
            ...
            <div className="flex gap-2">
              <Select
                ...
                onValueChange={(val) => {
                  if (val === '__new__') {
                    ...
                    setCredentialId(val === "__none__" ? "" : val)
              ...
              >
                ...
                <SelectContent className="bg-popover border-white/[0.08]">
                  ...
                  <SelectItem value="__new__">+ Add new key...</SelectItem>
                </SelectContent>
              </Select>
              {!showNewCred && (
                <button
                  ...
                  className="shrink-0 px-3 py-2.5 rounded-[10px] bg-primary/10 text-primary text-[12px] font-600 hover:bg-primary/20 transition-colors cursor-pointer border border-primary/20"
                >
                  + New
                </button>
              )}
            ...
            {showNewCred && (
              <div className="mt-3 p-4 rounded-[12px] border border-primary/15 bg-primary/5 space-y-3"
                style={{ animation: 'fade-in 0.2s ease-out' }}>
                <input
                  value={newCredName}
                ...
                <input
                  ...
                  className={`${inputClass} !bg-card font-mono text-[13px]`}
                ...
                <div className="flex gap-2 justify-end">
                  ...
                  <button
                    ...
                    className="px-4 py-1.5 rounded-[8px] bg-primary text-primary-foreground text-[12px] font-600 cursor-pointer border-none hover:brightness-110 transition-all disabled:opacity-40 shadow-[0_2px_8px_rgba(var(--primary-rgb),0.2)]"
                    style={{ fontFamily: 'inherit' }}
                  >
            ...
            return (
              <div key={field.key} className="mb-6">
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {tags.map((tag, i) => (
                    <span key={i} className="flex items-center gap-1.5 px-3 py-1.5 rounded-[8px] bg-primary/10 border border-primary/20 text-[12px] font-mono text-primary">
                      {tag}
                      <button
                        onClick={() => {
                        ...
                        className="ml-0.5 w-4 h-4 flex items-center justify-center rounded-full hover:bg-white/10 transition-colors cursor-pointer text-primary/50 hover:text-primary"
                      >
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 rounded-[10px] bg-primary/10 text-primary text-[12px] font-600 hover:bg-primary/20 transition-colors cursor-pointer border border-primary/20"
                  >
                    Add
                  </button>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-[12px] text-text-3 mt-0.5 flex items-center gap-1.5">
                    <span className={`w-2 h-2 rounded-full inline-block ${effectiveRunning ? 'bg-green-400 shadow-[0_0_6px_rgba(74,222,128,0.5)]' :
                      editing.status === 'error' ? 'bg-red-400' : 'bg-white/20'
                      }`} />
      ...
      <div className="flex gap-3 pt-4 border-t border-white/[0.04]">
        ...
        <button
          ...
          className="flex-1 py-3.5 rounded-[14px] border border-white/[0.08] bg-transparent text-text-2 text-[15px] font-600 cursor-pointer hover:bg-card transition-all"
          style={{ fontFamily: 'inherit' }}
        >
          Cancel
        </button>
        <button
          ...
          className="flex-1 py-3.5 rounded-[14px] border-none bg-primary text-primary-foreground text-[15px] font-600 cursor-pointer active:scale-[0.97] disabled:opacity-30 transition-all shadow-[0_4px_20px_rgba(var(--primary-rgb),0.25)] hover:brightness-110"
          style={{ fontFamily: 'inherit' }}
        >
